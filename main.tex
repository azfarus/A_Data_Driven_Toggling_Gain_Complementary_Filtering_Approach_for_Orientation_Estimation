...
\begin{table}[htbp]
    \centering
    \caption{Final Hyperparameters for Movement Prediction XGB Classifier}
    ...
    \begin{tabular}{|l|c||p{0.3\textwidth}|c|}
        \hline
            ...
            \texttt{reg\_lambda} & 9.1995 & \texttt{Accuracy (Test / Validation)} & 0.9956 / 0.9958 \\
...
\begin{table}[H]
\centering
\caption{Static error metrics when ML denoising is active.}
\label{tab:static_ml}
\begin{tabular}{l
  *{4}{ccc}
}
...
The tests without the machine learning module provides important insights into the comparative performance of different filters. The results have been summarized in \autoref{tab:dynamic_no_ml},\autoref{tab:quasi_no_ml} and \autoref{tab:static_no_ml}. The observations can be summarized as follows:
...
The evaluation with the machine learning (ML) based noise removal enabled reveals a different trend compared to the results without ML. The results have been summarized in \autoref{tab:dynamic_ml},\autoref{tab:quasi_ml} and \autoref{tab:static_ml}. The observations can be summarized as follows:
...
The computational demands of the proposed filter are noticeably higher compared to traditional orientation filters.Summary of the computational statistics is found in \autoref{tab:resource_consumption}. This limitation arises from multiple contributing factors:
