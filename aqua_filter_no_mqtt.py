...
aqua_filter = ahrs.filters.aqua.AQUA(frequency=sampling_rate)
...
f = open(f"data/{run_folder}/aqua.csv",mode="a+")
i=0
...
for _ in range(n):
    ...
    q = aqua_filter.updateMARG(q,g,a,m) 
    init_q+=q
...
print("Start of AQUA filtering")

aqua_filter.Dt = 1/285
while True:
    ...
    res = (Quaternion(init_q).inverse * Quaternion(q)).elements
    
    # if i%1000 == 0: print("res: ",res,"est ",q)

    timestamp_ms = int(time.time() * 1000)

    line = f"{timestamp_ms}, {res[0]}, {res[1]}, {res[2]}, {res[3]}\n"
